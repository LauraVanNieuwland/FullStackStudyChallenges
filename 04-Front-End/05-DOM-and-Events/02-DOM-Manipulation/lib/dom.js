...
module.exports = function runChallenges(check) {
  ...
  emailValue.setAttribute("value", "lauravannieuwland@gmail.com"); 
  ...
  const teamCount = () => {
    const numberOfTeams = document.querySelectorAll(".table tbody tr").length;
  ...
  list.insertAdjacentHTML('beforeend', '<tr><td>14</td><td>Team Laura </td><td>0</td></tr>');

  // Ex 7. Write some code to sum all points given to all teams
  //       Make the function summarizePoints() return it
  const summarizePoints = () => {
    let points = document.querySelectorAll("tbody tr td:last-child");
    let count = 0;
    points.forEach((point) => {
      let number = point.innerText;
  ...
  document.querySelectorAll(".table thead tr th").forEach(th => {
    th.style.cssText = "background-color: #DDF4FF";
  });
  // Ex 9. Remove the "Email:" label from the DOM


  const emailLabel = document.querySelector("div label");
  emailLabel.remove();
